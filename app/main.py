...
from models import User, RecoveryToken, Character, World # Import your User model
            ...
            if world_name:
                ...
                if existing_world:
                    ...
                    if existing_world.location != world_location:
                        ...
                        print(f"Updated world: {world_name}")
                else:
                    # Add new world
                    new_world = World(name=world_name, location=world_location)
                    db.add(new_world)
                    print(f"Added new world: {world_name}")

        db.commit()
        print("World data updated successfully from TibiaData.com.")

    except httpx.HTTPStatusError as e:
        print(f"HTTP error updating worlds from TibiaData: {e.response.status_code} - {e.response.text}")
        db.rollback()
    except httpx.RequestError as e:
        print(f"Network error updating worlds from TibiaData: {e}")
        db.rollback()
    except Exception as e:
        print(f"An unexpected error occurred while updating worlds: {e}")
        db.rollback()
...
async def on_startup():
    # Initialize FastAPI-Limiter with Redis (commented out as per user request for now)
...
async def create_character(
    ...
    The validation_hash is automatically generated by the model's event listener.
    """
    user_id = request.session.get('user_id')
    if not user_id:
        return RedirectResponse(url="/login", status_code=status.HTTP_303_SEE_OTHER)
    ...
    # Validate character name uniqueness (Tibia.com name is unique globally)
    existing_character = db.query(Character).filter(Character.name == name).first()
    if existing_character:
        error_message = f"Character '{name}' already exists. Tibia.com character names must be unique."

    # Validate world_id
    world = db.query(World).filter(World.id == world_id).first()
    if not world:
        error_message = "Invalid world selected."

    if error_message:
        # Re-fetch characters and worlds to re-render the account_recovery page with error
        ...
        return templates.TemplateResponse(
            "account_recovery.html",
            {"request": request, "logged_in_user": user, "characters": characters, "worlds": worlds, "error": error_message}
        )
    ...
    try:
        new_character = Character(
            ...
            vocation=vocation,
            user_id=user.id,
            world_id=world_id # Assign the world_id
        )
        db.add(new_character)
        # Flush the session to ensure the user relationship is loaded for the event listener
        # before the commit, so it can generate the validation_hash using user.username.
        db.flush()
        db.commit()
        db.refresh(new_character)

        return RedirectResponse(
            url=f"/characters/{new_character.name}?message=Character '{new_character.name}' added successfully! Put the generated hash in your character comment to validate.",
            status_code=status.HTTP_303_SEE_OTHER
        )
    except IntegrityError as e:
        db.rollback()
        # This might catch unique constraint violations not caught by initial checks
        error_message = f"A character with the name '{name}' already exists or another database integrity error occurred."
        print(f"Integrity Error during character creation: {e}")
    except Exception as e:
        db.rollback()
        error_message = f"An unexpected error occurred during character creation: {e}"
        print(f"Error during character creation: {e}")

    # If an error occurred, re-render the account_recovery page with error message
    characters = db.query(Character).filter(Character.user_id == user.id).order_by(Character.name).all()
    worlds = db.query(World).order_by(World.name).all()
    return templates.TemplateResponse(
        "account_recovery.html",
